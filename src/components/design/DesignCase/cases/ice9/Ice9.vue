<template>
  <div class="ice9">
    <GridRow
      :noRight="true"
      :isBottom="true"
      :mobileHidden="true">
      <template slot="one"></template>
      <template slot="two">
        <ArrowLink
          class="arrow-link">
          <template slot="text">
            ux/ui design for security system
          </template>
          <template slot="caption">
            <span
              class="is-small is-margin-right-x5"
              v-for="topic in project.topics"
              :key="topic">
              {{ topic }}
            </span>
          </template>
        </ArrowLink>
      </template>
    </GridRow>

    <GridRow
      :noRight="true"
      :isBottom="true"
      :mobileHidden="true">
      <template slot="one">
        <ArrowLink class="arrow-link is-aligned-right">
          <template slot="text">overview</template>
        </ArrowLink>
      </template>
      <template slot="two">
        <div class="design-container">
          <h2 class="design-header-mobile">overview</h2>
          <Overview />
        </div>
      </template>
    </GridRow>

    <GridRow
      :noRight="true"
      :isBottom="true"
      :mobileHidden="true">
      <template slot="one">
        <ArrowLink class="arrow-link is-aligned-right">
          <template slot="text">logo</template>
        </ArrowLink>
      </template>
      <template slot="two">
        <div class="design-container">
          <h2 class="design-header-mobile">logo</h2>
          <ImageGallery
            folder="ice"
            :images="logoImages" />
        </div>
      </template>
    </GridRow>

    <GridRow
      :noRight="true"
      :isBottom="true"
      :mobileHidden="true">
      <template slot="one">
        <ArrowLink class="arrow-link is-aligned-right">
          <template slot="text">ui elements</template>
        </ArrowLink>
      </template>
      <template slot="two">
        <div class="design-container">
          <h2 class="design-header-mobile">ui elements</h2>
          <ImageGallery
            folder="ice"
            :images="uiImages" />
        </div>
      </template>
    </GridRow>

    <GridRow
      :noRight="true"
      :isBottom="true"
      :mobileHidden="true">
      <template slot="one"></template>
      <template slot="two">
        <ArrowLink
          class="arrow-link">
          <template slot="text">
            more content will be soon...
          </template>
        </ArrowLink>
      </template>
    </GridRow>
  </div>
</template>

<script lang="ts">
import Vue from 'vue';
import {Routes} from '@/router';
import {AnimationService} from '@/shared/services/animation.service';
import ImageGallery from '@/components/design/DesignCase/cases/common/ImageGallery/ImageGallery.vue';
import Overview from '@/components/design/DesignCase/cases/ice9/Overview/Overview.vue';

export default Vue.extend({
  name: 'Ice9',
  components: {
    ImageGallery,
    Overview
  },
  data() {
    return {
      Routes,
      project: this.$attrs,
      logoImages: [
        'logo-white-caption.png',
        'logo-purple-caption.png'
      ],
      uiImages: [
        'ice-elements.png'
      ]
    }
  },
  mounted() {
    AnimationService.removeCover();
  }
});
</script>

<style lang="scss" scoped>

</style>
